...
// cambios) — este contexto lleva la cuenta del tiempo Y, cuando el
// pomodoro llega a 0 de verdad, guarda el avance del curso en el
// Horario (ver tick() más abajo), sin importar qué pantalla esté
// montada en ese momento.
...
export function PomodoroProvider({ children }) {
  ...
  // Refs para que tick() (memoizado una sola vez) siempre lea el
  // subject/day más reciente sin tener que recrearse cada vez que
  // cambian — mismo patrón que ya usa onCompleteRef en useCountdown.js.
  const subjectRef = useRef("");
  const dayRef = useRef("");
  useEffect(() => { subjectRef.current = subject; }, [subject]);
  useEffect(() => { dayRef.current = day; }, [day]);

  // Pantalla (típicamente Horario) que quiere enterarse en el momento
  // en que un pomodoro termina de verdad, para sincronizar su propio
  // estado sin esperar a un remount. Es opcional: el avance real del
  // progreso (más abajo) NO depende de que exista este callback.
  ...
  const tick = useCallback((endTs) => {
    ...
    if (remaining <= 0) {
      ...
      // Guardamos el avance del curso en localStorage YA, acá mismo,
      // sin importar qué pantalla esté montada en este momento — así
      // el progreso no se pierde si el pomodoro termina mientras el
      // usuario está estudiando en Mi Estudio en vez de en Horario.
      const resultado = subjectRef.current
        ? avanzarProgresoPomodoro({ day: dayRef.current, subject: subjectRef.current })
        : null;
  ...
  const iniciar = useCallback((minutos, nuevaLabel, nuevoSubject, nuevoDay) => {
    ...
    const dayFinal = nuevoDay !== undefined ? nuevoDay : day;
    if (nuevoDay !== undefined) setDay(nuevoDay);
    setEndTimestamp(nuevoEnd);
    guardarPomodoroCompartido({ endTimestamp: nuevoEnd, running: true, label: labelFinal, subject: subjectFinal, day: dayFinal });
