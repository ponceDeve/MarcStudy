...
// Guarda qué temas ya se programaron, aparte del log de repasos. Así, si
// se elimina un repaso, "Recomendado de hoy" no retrocede en la rotación.
export const ROTACION_HISTORIAL_KEY = "mi_estudio_rotacion_historial";
export function leerHistorialRotacion() {
  try {
    const raw = localStorage.getItem(ROTACION_HISTORIAL_KEY);
    return raw ? JSON.parse(raw) : [];
  } catch (e) {
    console.error("Error leyendo historial de rotación:", e);
    return [];
  }
}
function agregarAHistorialRotacion({ subject, day = "", tema = "" }) {
  try {
    const historial = leerHistorialRotacion();
    const yaEsta = historial.some(
      (h) => h.subject === subject && h.tema === tema && h.day === day
    );
    if (yaEsta) return;
    historial.push({ subject, day, tema });
    localStorage.setItem(
      ROTACION_HISTORIAL_KEY,
      JSON.stringify(historial)
    );
  } catch (e) {
    console.error("Error guardando historial de rotación:", e);
  }
}
...
export function registrarCursoCompletado({
  ...
  guardarLog(log);
  agregarAHistorialRotacion({ subject, day, tema });
  return log;
}
// ── Marcar repaso hecho ─────────────────────────────────────────────────
...
export function eliminarRepaso(id) {
  const log = leerLog();
  const eliminada = log.find((e) => e.id === id);
  if (eliminada) {
    agregarAHistorialRotacion(eliminada);
  }
...
export function clasificarRepasos(log) {
  ...
  log.forEach((entrada) => {
    ...
    if (diff <= Math.max(...REPASO_INTERVALOS)) {
      proximos.push({
        entrada,
        fecha: prox.fecha,
        diff,
