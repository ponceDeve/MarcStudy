...
function obtenerRaizPalabra(palabra){
  ...
  for(const terminacion of terminaciones){
    if(p.endsWith(terminacion)&&p.length-terminacion.length>=4){
...
function palabrasCoinciden(palabraConsulta,palabraTexto){
  ...
  if(raizA.length>=4&&raizB.length>=4){
    if(raizA.startsWith(raizB)||raizB.startsWith(raizA)) return true;
  }
  return false;
}
function puntajeLexico(texto,consulta){
  ...
  for(const palabraConsulta of palabrasConsulta){
    ...
    for(const palabraTexto of palabrasTexto){
      if(normalizarTexto(palabraConsulta)===normalizarTexto(palabraTexto)){
...
function ResaltarCoincidencia({texto,query}){
  ...
  if(indice!==-1){
    return <>
      ...
      <span className="search-match">{textoOriginal.slice(indice,indice+busqueda.length)}</span>
  ...
  return partes.map((parte,index)=>{
    ...
    const coincide=palabrasConsulta.some(p=>palabrasCoinciden(p,limpio));
...
function armarFragmentoExplicacion(explicacion,query){
  ...
  return {
    fragmento,
    indice:buscarPosicion(fragmento,queryLimpia),
...
function buscarEnContenidoTema(contenidoTema,query){
  ...
  for(const punto of contenidoTema){
    const matchTexto=buscarCoincidencia(punto.texto,q);
    if(matchTexto){
      resultados.push({
        type:"contenido",
        puntoId:punto.id,
    ...
    const matchExplicacion=buscarCoincidencia(punto.explicacion,q);
    if(matchExplicacion){
      resultados.push({
        type:"contenido",
        puntoId:punto.id,
        ...
        matchText:q,
        _score:puntajeDeTexto(punto.explicacion,q)
      });
    }
  }
...
async function buscarFuertes(query){
  ...
  if(literal.cursos.length>0||literal.temas.length>0){
    return {
      ...
      temas:literal.temas,
      coincidenciasExactas:[...literal.cursos,...literal.temas],
      coincidenciaLiteral:literal.coincidenciaLiteral,
      esLiteral:true
    };
  }
  try{
...
export default function SearchModal({
  ...
  const [cursosAbiertos,setCursosAbiertos]=useState(new Set());
  ...
  useEffect(()=>{
    ...
    if(fuertes.temas.length>0){
      setCursosAbiertos(
        new Set(
          fuertes.temas.map(tema=>tema.curso)
        )
      );
      return;
    }
    ...
    return (
      ...
      >
        <div className="search-course-row">
          ...
          >
            <i
              className={`fa-solid ${estaAbierto?"fa-minus":"fa-plus"}`}
            />
          </button>
        </div>
        <div
                    ...
                    <p className="search-result-item__tema">
                      {r.campo==="texto"?(
                        ...
                        <ResaltarFragmento
                          ...
                          largoCoincidencia={fragmento.largo}
                        />
                      )}
                    </p>
                  </button>
