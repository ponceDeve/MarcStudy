...
function obtenerMensajeSegunCurso(curso, porcentaje, usadosPorCurso) {
  ...
  const disponibles = mensajes.map((_, indice) => indice).filter((indice) => !usados.includes(indice));
...
function PreguntaResultado({ item, numero, abierta, onToggle }) {
  ...
  return (
    <li className={`resultados-examen__pregunta is-${estado} ${abierta ? "is-abierta" : ""}`}>
      <button type="button" className="resultados-examen__pregunta-header" onClick={onToggle}>
        {ICONO_ESTADO[estado]}
...
export default function ResultadosExamenPage({ resultados, area, nombreArea, onSalir }) {
  const { puntajeTotal, grupos } = resultados;
  const [preguntaAbiertaId, setPreguntaAbiertaId] = useState(null);
  const [cursoSeleccionado, setCursoSeleccionado] = useState(grupos[0]?.curso ?? null);
  const usadosPorCursoRef = useRef({});
  const mensajesPorCursoRef = useRef({});

  function alternarPregunta(id) {
    setPreguntaAbiertaId((actual) => actual === id ? null : id);
  }
  ...
  const grupoSeleccionado = grupos.find((grupo) => grupo.curso === cursoSeleccionado);
  ...
  function obtenerMensajeDelCurso(grupo) {
    ...
    const porcentaje = grupo.items.length
      ? (grupo.items.filter((item) => item.estado === "correcta").length / grupo.items.length) * 100
      : 0;
  ...
  return (
    <div className="resultados-examen container">
      <div className="resultados-examen__resumen">
        <h1 className="resultados-examen__puntaje">{puntajeTotal.toFixed(2)}</h1>
        <p className="resultados-examen__subtitulo">
          Área {area} · {nombreArea} · {totalCorrectas}/{totalPreguntas} correctas
        </p>
      </div>
      ...
      >
        {grupos.map((grupo) => (
          <option key={grupo.curso} value={grupo.curso}>
            {grupo.nombre}
          </option>
        ))}
      </select>
          ...
          <ul className="resultados-examen__lista">
            {grupoSeleccionado.items.map((item, index) => (
              <PreguntaResultado
                ...
                item={item}
                numero={index + 1}
                abierta={preguntaAbiertaId === item.pregunta.id}
                onToggle={() => alternarPregunta(item.pregunta.id)}
              />
            ))}
          </ul>
        </section>
